...
<Aside>Bobby is not enabled by default due to cached ComputerCraft monitors not rendering their display and because it uses a *lot* of storage on the client.</Aside>
...
<Aside>Nvidium hasn't been added by default as it only works for NVIDIA GeForce GTX 16xx cards and newer (GeForce RTX), and because its quite an experimental mod.</Aside>
